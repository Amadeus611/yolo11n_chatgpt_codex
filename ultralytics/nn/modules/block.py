...
class PartialConv(nn.Module):
    ...
    def __init__(self, c1: int, n_div: int = 4):
        """初始化部分空间卷积与逐点通道混合。"""
        super().__init__()
        self.c_partial = max(c1 // n_div, 1)
        self.c_untouched = c1 - self.c_partial
        self.cv1 = Conv(self.c_partial, self.c_partial, 3, 1)
        self.cv2 = Conv(c1, c1, 1, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """只对部分通道应用空间卷积。"""
        x1, x2 = torch.split(x, [self.c_partial, self.c_untouched], dim=1)
        x = torch.cat((self.cv1(x1), x2), dim=1) if self.c_untouched else self.cv1(x1)
        return self.cv2(x)
...
class PConvBottleneck(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, shortcut: bool = True, e: float = 0.5, n_div: int = 4):
        """初始化轻量瓶颈块。"""
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = PartialConv(c_, n_div=n_div)
...
class C3k2Lite(C2f):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(PConvBottleneck(self.c, self.c, shortcut, e=1.0, n_div=n_div) for _ in range(n))
...
class DySample(nn.Module):
    ...
    def __init__(self, scale: int = 2):
        """以零偏移初始化 DySample，使训练初期接近常规上采样。"""
        super().__init__()
        self.scale = scale
        self.offset = None

    def _make_offset(self, c: int, device: torch.device) -> nn.Conv2d:
        """在输入通道数确定后延迟创建偏移生成器。"""
        offset = nn.Conv2d(c, 2 * self.scale * self.scale, 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """使用小幅可学习采样偏移上采样特征。"""
        b, c, h, w = x.shape
        offset = self.offset if self.offset is not None else self._make_offset(c, x.device)
        delta = F.pixel_shuffle(offset(x), self.scale).tanh() * 0.25
        h2, w2 = h * self.scale, w * self.scale
        yy, xx = torch.meshgrid(
            torch.arange(h2, device=x.device, dtype=x.dtype),
            torch.arange(w2, device=x.device, dtype=x.dtype),
            indexing="ij",
        )
        grid_x = ((xx + 0.5) / self.scale) / w * 2 - 1
        grid_y = ((yy + 0.5) / self.scale) / h * 2 - 1
        delta_x = delta[:, 0].mul(2 / max(w, 1))
        delta_y = delta[:, 1].mul(2 / max(h, 1))
        grid = torch.stack((grid_x + delta_x, grid_y + delta_y), dim=-1)
        return F.grid_sample(x, grid, mode="bilinear", padding_mode="border", align_corners=False)


class SCSPDRCF(nn.Module):
    """面向 YOLO11 Neck 特征的语义校准浅层细节回放模块。"""

    def __init__(self, ch: list[int], alpha: float = 0.1, hidden: int | None = None):
        """以弱残差注入方式初始化 SCSP + DRCF。"""
        super().__init__()
        c_shallow, c_semantic, c_target = ch
        hidden = hidden or max(min(c_target // 2, 128), 16)
        self.shallow_proj = Conv(c_shallow, hidden, 3, 1)
