...
class BboxLoss(nn.Module):
    ...
    def __init__(
        ...
        self.use_sndq = use_sndq
        self.sndq_gamma = sndq_gamma
        self.sndq_tau = sndq_tau
        self.sndq_c = sndq_c
    ...
    def _box_wasserstein_distance(box1: torch.Tensor, box2: torch.Tensor) -> torch.Tensor:
        """计算 HBB 二维高斯表示之间的 Wasserstein 距离。"""
        b1 = xyxy2xywh(box1)
        b2 = xyxy2xywh(box2)
        center_dist = (b1[..., :2] - b2[..., :2]).pow(2).sum(-1, keepdim=True)
        size_dist = (b1[..., 2:] - b2[..., 2:]).pow(2).sum(-1, keepdim=True) / 4
        return torch.sqrt(center_dist + size_dist + 1e-9)

    @staticmethod
    def _nearest_same_class_gt(
        target_gt_idx: torch.Tensor,
        gt_labels: torch.Tensor,
        gt_bboxes: torch.Tensor,
        fg_mask: torch.Tensor,
    ) -> torch.Tensor:
        """为每个正样本查找最近的同类非匹配 GT 框。"""
        nearest = torch.zeros_like(gt_bboxes.new_zeros((fg_mask.sum(), 4)))
        offset = 0
        for batch_i in range(fg_mask.shape[0]):
            pos_mask = fg_mask[batch_i]
            num_pos = int(pos_mask.sum())
            if num_pos == 0:
                continue

            valid_gt = gt_bboxes[batch_i].sum(-1).gt(0.0)
            if valid_gt.sum() < 2:
                offset += num_pos
                continue

            gt_boxes_i = gt_bboxes[batch_i, valid_gt]
            gt_labels_i = gt_labels[batch_i, valid_gt, 0]
            matched_idx = target_gt_idx[batch_i, pos_mask].clamp(min=0).long()
            matched_boxes = gt_boxes_i[matched_idx]
            matched_labels = gt_labels_i[matched_idx]
            centers = xyxy2xywh(gt_boxes_i)[..., :2]
            matched_centers = xyxy2xywh(matched_boxes)[..., :2]
            same_class = gt_labels_i.unsqueeze(0).eq(matched_labels.unsqueeze(1))
            not_self = torch.arange(gt_boxes_i.shape[0], device=gt_boxes_i.device).unsqueeze(0).ne(matched_idx.unsqueeze(1))
            candidate_mask = same_class & not_self
            distance = torch.cdist(matched_centers, centers)
            distance = distance.masked_fill(~candidate_mask, torch.inf)
            has_neighbor = torch.isfinite(distance).any(1)
            if has_neighbor.any():
                nearest_idx = distance[has_neighbor].argmin(1)
                nearest[offset : offset + num_pos][has_neighbor] = gt_boxes_i[nearest_idx]
            offset += num_pos
        return nearest
    ...
    def _sndq_loss(
        ...
        iou = bbox_iou(pred_bboxes, target_bboxes, xywh=False).clamp(0, 1)
        ...
        lambda_scale = torch.exp(-torch.sqrt(target_wh.prod(-1, keepdim=True)) / self.sndq_tau).clamp(0, 1)
        quality = (1 - lambda_scale) * iou + lambda_scale * nwd

        if neighbor_valid.any():
            neighbor_px = neighbor_bboxes * stride
            center_dist = (xyxy2xywh(target_px)[..., :2] - xyxy2xywh(neighbor_px)[..., :2]).norm(dim=-1, keepdim=True)
            ...
            penalty = self.sndq_gamma * rho * (leak - self.sndq_margin).clamp(min=0)
        else:
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.bbox_loss = BboxLoss(
            m.reg_max,
            use_sndq=bool(getattr(h, "sndq", False) or getattr(model, "yaml", {}).get("sndq", False)),
            sndq_gamma=float(getattr(h, "sndq_gamma", getattr(model, "yaml", {}).get("sndq_gamma", 0.05))),
            sndq_tau=float(getattr(h, "sndq_tau", getattr(model, "yaml", {}).get("sndq_tau", 32.0))),
